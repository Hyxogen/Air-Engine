...
namespace platform {
	namespace windows {
		...
		uint32 WindowsKeyMap::GetSysCode(uint32 keyCode) {
			for (uint32 i = 0; winKeyPairs[i].m_SysCode != engine::io::KEY_UNKNOWN; i++) {
		...
		uint32 WindowsKeyMap::GetKeyCode(uint32 sysCode) {
			for (uint32 i = 0; winKeyPairs[i].m_SysCode != engine::io::KEY_UNKNOWN; i++) {
